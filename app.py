...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/HW5becclest"
# Provided:
...
# Association table
on_list = db.Table('on_list', db.Column('item_id', db.Integer, db.ForeignKey(
...
class TodoList(db.Model):
    ...
    items = db.relationship('TodoItem', secondary=on_list, backref=db.backref(
...
class TodoListForm(FlaskForm):
    name = StringField("What is the title of this TODO List?",
                       validators=[Required()])
    items = TextAreaField(
        "Enter your TODO list items in the following format: Description, Priority -- separated by newlines")
...
class UpdateInfoForm(FlaskForm):
    newPriority = StringField(
        "What is the new priority of this item?", validators=[Required()])
    submit = SubmitField('Update')
...
def get_or_create_todolist(title, item_strings=[]):
    list = TodoList.query.filter_by(title=title).first()
    if not list:
...
def delete(lst):
    list = TodoList.query.filter_by(id=lst).first()  # Just like update!
    db.session.delete(list)
    db.session.commit()
    flash("Successfully deleted {}".format(list.title))
    return redirect(url_for('all_lists'))
